...
def _unescape_cue_value(value: str) -> str:
    """Decode the limited escaping emitted by PackBuilder without evaluating input."""
    return value.replace("\\\\", "\0").replace('\\"', '"').replace("\0", "\\")


def _read_quoted_value(content: str, key: str) -> str:
    """Read ``key=\"...\"`` while preserving escaped quote characters."""
    match = re.search(rf'^{re.escape(key)}="((?:\\.|[^"\\])*)"\s*$', content, re.MULTILINE)
    return _unescape_cue_value(match.group(1)) if match else ""


def _read_quoted_array(content: str, key: str) -> List[str]:
    """Read a simple quoted string array as generated by PackBuilder."""
    match = re.search(rf'^{re.escape(key)}=\[(.*)\]\s*$', content, re.MULTILINE)
    if not match:
        return []
    return [_unescape_cue_value(value) for value in re.findall(r'"((?:\\.|[^"\\])*)"', match.group(1))]
...
class ProjectManager:
    ...
    def load_from_pack_folder(cls, pack_dir: Path) -> PipelineState:
        ...
        for txt_path in txt_files:
            ...
            caption = _read_quoted_value(content, "caption")
            image_name = _read_quoted_value(content, "image")
            ...
            char_names = _read_quoted_array(content, "dub_characters")
    ...
    def _parse_pack_info_ini(ini_path: Path, pack_info: PackInfo):
        ...
        try:
            ...
            title = _read_quoted_value(text, "title")
            if title:
                pack_info.title = title

            icon = _read_quoted_value(text, "icon")
            if icon:
                pack_info.icon = icon

            authors = _read_quoted_array(text, "authors")
            if authors:
