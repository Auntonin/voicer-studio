...
class ProgressPanel(QWidget):
    def __init__(self, parent=None):
        ...
        # ── Top header ────────────────────────────────────────────────
        header = QHBoxLayout()
        lbl_title = QLabel("Pipeline Progress")
        lbl_title.setStyleSheet(f"color:{COLORS['text_primary']};font-weight:bold;font-size:10pt;")
        ...
        self.steps = [
            (PipelineStep.AUDIO_EXTRACT, "Extract"),
            (PipelineStep.VAD,           "VAD"),
            (PipelineStep.DIARIZATION,   "Diarize"),
            (PipelineStep.TRANSCRIPTION, "Transcribe"),
            (PipelineStep.VOICE_SEPARATION,"Separate"),
            (PipelineStep.CLIP_GENERATION,"Clips"),
            (PipelineStep.FRAME_EXTRACTION,"Frames"),
            (PipelineStep.BACKING_TRACK, "BG Track"),
            (PipelineStep.PACK_BUILD,    "Pack"),
            (PipelineStep.VALIDATION,    "Validate"),
        ]

        self.step_widgets = {}
        for i, (step, text) in enumerate(self.steps):
            col = QVBoxLayout()
        ...
        self.lbl_current_step = QLabel("Ready")
    ...
    def reset(self):
        ...
        self.lbl_sub_pct.setText("0%")
        self.lbl_sub_detail.setText("")
        self.lbl_current_step.setText("Ready")
        self.start_time = None
        self.timer.stop()
